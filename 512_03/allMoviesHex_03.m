...
% 
setGlobals512_03
[algoEleDict, thresEleDict, discrepancyNeurons] = getChosenElectrodesDicts(3);
...
for NEURON_ID = discrepancyNeurons'
    ...
    detectionThres = SPIKE_DETECTION_THRES_DICT(NEURON_ID);
    adjacentElectrodes = getAdjacentElectrodes(recEle, ELE_MAP_OBJ);
    path = ['C:\studia\dane_skrypty_wojtek\ks_functions\512_03\graph\all_movies\' num2str(NEURON_ID) '\'];
    mkdir(path)
    for movieIdx = 1:size(allMoviesResultStructs,1)
        ...
        plotHexagonally2(adjacentElectrodes, resultStructs, eiSpike, eiSpikeAmp, detectionThres, thresEleDict(NEURON_ID), algoEleDict(NEURON_ID))
...
%%
% save('512_all_movies_3', 'allNeuronsDict')
...
SPIKE_DET_MARG = 0;
HOW_MANY_SPIKES = 25;
PATH_ROOT = 'C:\studia\dane_skrypty_wojtek\ks_functions\holistic_algo\graph3\';
addJava
setGlobals
% global NEURON_IDS
...
for i = 1:length(NEURON_IDS);
    ...
    for movie = movieIdx
        ...
        for thresholdArrowIdx = 1:length(THRESHOLDS)
            meanAlgoSpike = plotTracesAndMeasureArrow(NEURON_ID, movie, thresholdArrowIdx, movieMeasureMat, ...
                movieArtIDs, movieExclIDs, movieSpikeIDs, bestThreshold, AMP_DICT(NEURON_ID), SAMPLES_LIM, THRESHOLDS, fullPath);
        end

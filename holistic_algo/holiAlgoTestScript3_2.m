...
SPIKE_DET_MARG = 0;
HOW_MANY_SPIKES = 25;
PATH_ROOT = 'C:\studia\dane_skrypty_wojtek\ks_functions\holistic_algo\traces_measure_5_groups\';
addJava
setGlobals
% global NEURON_IDS
...
for i = 1:length(NEURON_IDS);
    ...
    for movie = MOVIES
        ...
        for thresholdArrowIdx = 1%:length(THRESHOLDS)
            meanAlgoSpike = plotTracesAndMeasureArrow2(NEURON_ID, movie, thresholdArrowIdx, movieMeasureMat, ...
                movieArtIDs, movieExclIDs, movieSpikeIDs, bestThreshold, AMP_DICT(NEURON_ID), SAMPLES_LIM, THRESHOLDS, fullPathMovie, [0 300], movieIdx);
        end

%%%%%%%
%         for thresholdArrowIdx = 4%1:length(THRESHOLDS)
%             meanAlgoSpike = plotTracesAndMeanSpikes(NEURON_ID, movie, thresholdArrowIdx, movieMeasureMat, ...
%                 movieArtIDs, movieExclIDs, movieSpikeIDs, bestThreshold, NEURON_AMP, SAMPLES_LIM, THRESHOLDS, [0 30], PATH, movieIdx);
%         end
    end
%     figure

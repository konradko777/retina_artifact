function [ NumberOfDRW ] = mgrNumberOfDisqualifyingResultantWaveforms(Waveforms, Artifact, QuantTh, Active)
    ...
    resultantWaveforms(:,41:end) = []; % We use only 40 first samples
    resultantWaveforms(:,1:11) = []; % We skip 8 first samples (due to stimulation)
    % 150 us -> 8
    % 300 us -> 11 (albo 10...)
    % Aaaaaaaaaaaaaa!!!
    ...
    for iQWave = 1:size(quantizedWaveforms,1)
        ...
        sequence(sequence == 0) = [];
        if ~isempty(sequence) && sequence(1) == 1 && Active(iQWave) == 1
            nDRW = nDRW + 1;
        end
    end
